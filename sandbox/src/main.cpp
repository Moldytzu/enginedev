...
public:
    void Start()
    {
        ...
        Transform.Scale(glm::vec3(2,1,2))->Translate(glm::vec3(0,-0.5,0));
    }
...
public:
    ...
    void Update()
    {
        ...
        if (Engine::Input::GlobalInputManager->Keys[GLFW_KEY_S])
        {
            float speed = 100 * Engine::Render::GlobalRenderer->DeltaTime;
            Engine::Render::GlobalRenderer->CameraTransform.Rotate(Engine::Core::Random::Float(0, 1) * speed, Engine::Core::Random::Float(0, 1) * speed, Engine::Core::Random::Float(0, 1) * speed); // rotate randomly
        }
    }
...
void Engine::Core::Application::Start()
{
    ...
    Engine::Render::GlobalRenderer->CameraTransform.Translate(glm::vec3(0, 0.5, -2)); // translate the camera up and back
}
